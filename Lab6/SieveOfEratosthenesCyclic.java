...
class SharedCounter{
	...
	public SharedCounter(int size) {
		this.prime = new boolean[size +1]; //arxikopoiisi prime[]
		
		for(int i = 2; i <= size; i++)
			prime[i] = true; 	          // gemisma prime[]	
...
class SieveThreads extends Thread {
    ...
    public SieveThreads(int id, int numThreads, int size, int limit, SharedCounter counter) {
        this.size = size;
    ...
    public void run() {
    	...
    	for(int p = myId; p < limit; p += numThreads) {
